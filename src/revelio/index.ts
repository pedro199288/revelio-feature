...
export class Revelio {
  ...
  private dialogPadding: RevelioOptions["dialogPadding"] = defaultOptions.dialogPadding
  private dialogMaxWidth: RevelioOptions["dialogMaxWidth"] = defaultOptions.dialogMaxWidth
  private dialogMaxHeight: RevelioOptions["dialogMaxHeight"] = defaultOptions.dialogMaxHeight
  private dialogMargin: RevelioOptions["dialogMargin"] = defaultOptions.dialogMargin
  private prevBtnText: RevelioOptions["prevBtnText"] = defaultOptions.prevBtnText
  private nextBtnText: RevelioOptions["nextBtnText"] = defaultOptions.nextBtnText
  private skipBtnText: RevelioOptions["skipBtnText"] = defaultOptions.skipBtnText
  private doneBtnText: RevelioOptions["doneBtnText"] = defaultOptions.doneBtnText
  private showPrevBtn: RevelioOptions["showPrevBtn"] = defaultOptions.showPrevBtn
  private showNextBtn: RevelioOptions["showNextBtn"] = defaultOptions.showNextBtn
  private showSkipBtn: RevelioOptions["showSkipBtn"] = defaultOptions.showSkipBtn
  private showDoneBtn: RevelioOptions["showDoneBtn"] = defaultOptions.showDoneBtn
  ...
  constructor({
    ...
    const element = typeof rootElement === 'string' ? document.querySelector(rootElement) : rootElement
    if (rootElement && !element) throw new Error(`Element ${element} not found`)
    if (element && !(element instanceof HTMLElement)) throw new Error(`Element ${element} is not an HTMLElement`)

    this.rootElement = element ?? document.body
  }

  private getStepElement(element: string | HTMLElement): HTMLElement {
    const e = typeof element === 'string' ? document.querySelector(element) : element
    if (!e) throw new Error(`Element ${element} not found`)
    if (!(e instanceof HTMLElement)) throw new Error(`Element ${element} is not an HTMLElement`)
    return e
  }

  private setStepProps() {
    const stepOptions = this.journey[this.currentIndex]?.options
    this.dialogPadding = stepOptions?.dialogPadding ?? this.baseConfig.dialogPadding ?? this.dialogPadding
    this.dialogMaxWidth = stepOptions?.dialogMaxWidth ?? this.baseConfig.dialogMaxWidth ?? this.dialogMaxWidth
    this.dialogMaxHeight = stepOptions?.dialogMaxHeight ?? this.baseConfig.dialogMaxHeight ?? this.dialogMaxHeight
    this.dialogMargin = stepOptions?.dialogMargin ?? this.baseConfig.dialogMargin ?? this.dialogMargin
    this.prevBtnText = stepOptions?.prevBtnText ?? this.baseConfig.prevBtnText ?? this.prevBtnText
    this.nextBtnText = stepOptions?.nextBtnText ?? this.baseConfig.nextBtnText ?? this.nextBtnText
    this.skipBtnText = stepOptions?.skipBtnText ?? this.baseConfig.skipBtnText ?? this.skipBtnText
    this.doneBtnText = stepOptions?.doneBtnText ?? this.baseConfig.doneBtnText ?? this.doneBtnText
    this.showPrevBtn = stepOptions?.showPrevBtn ?? this.baseConfig.showPrevBtn ?? this.showPrevBtn
    this.showNextBtn = stepOptions?.showNextBtn ?? this.baseConfig.showNextBtn ?? this.showNextBtn
    this.showSkipBtn = stepOptions?.showSkipBtn ?? this.baseConfig.showSkipBtn ?? this.showSkipBtn
    this.showDoneBtn = stepOptions?.showDoneBtn ?? this.baseConfig.showDoneBtn ?? this.showDoneBtn
    this.onStart = stepOptions?.onStart ?? this.baseConfig.onStart ?? this.onStart
  ...
  private renderStepDialog(step: JourneyStep, elementPosition: { top: number, left: number, }, elementDimensions: { width: number, height: number }) {
    ...
    const dialogLeft = Math.min(elementPosition.left + elementDimensions.width, rootElementWidth - dialogBorderBoxWidth)
    const dialogTop = Math.min(elementPosition.top + elementDimensions.height, rootElementHeight - dialogBorderBoxHeight)
